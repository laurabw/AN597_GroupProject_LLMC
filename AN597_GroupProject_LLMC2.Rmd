...
Let's load in the data. We're going to use the Kamala and Cooper dataset, but in this particular set, we have simplified it down to only three families and only the male and femal body size and brain size. 
```{r}
...
```{r}
library(klaR)
partimat(Family ~ Body_mass_male_mean + Body_mass_female_mean + Mass_Dimorphism, data = d, method = "lda")
```
